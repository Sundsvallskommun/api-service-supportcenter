...
class EndOfLeaseStatisticsResource {
	...
	@Operation(summary = "How the end of lease batches of a municipality have gone",
		description = "Answers with what the batches of a window add up to and with the same counts one batch at a time, so that a batch worth looking closer at can be picked out and read by its id. The batches come back one page at a time, while the counts at the top cover the whole window. The window is the month up to today when none is given, and the one that was answered is part of the answer. The counts are the state the computers are in now. A computer that failed and then went through counts as sent",
		responses = {
	...
	ResponseEntity<EndOfLeaseStatisticsResponse> getEndOfLeaseStatistics(
		...
		@ParameterObject @Valid final EndOfLeaseStatisticsParameters parameters) {

		return ok(endOfLeaseService.getStatistics(municipalityId, parameters));
	}
}
