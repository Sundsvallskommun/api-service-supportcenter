...
class EndOfLeaseBatchResourceTest {
	...
	void getEndOfLeaseBatch() {
		...
		when(endOfLeaseServiceMock.getBatchStatus(MUNICIPALITY_ID, BATCH_ID, null)).thenReturn(EndOfLeaseBatchStatusResponse.create()
			.withId(BATCH_ID)
			.withExternalBatchId(EXTERNAL_BATCH_ID)
			.withCreated(created)
			.withTotal(2)
		...
		verify(endOfLeaseServiceMock).getBatchStatus(MUNICIPALITY_ID, BATCH_ID, null);
	}

	@Test
	void getEndOfLeaseBatchFilteredOnState() {

		when(endOfLeaseServiceMock.getBatchStatus(MUNICIPALITY_ID, BATCH_ID, List.of("FAILED"))).thenReturn(EndOfLeaseBatchStatusResponse.create()
			.withId(BATCH_ID)
			.withTotal(982)
			.withFailed(3)
		...
		webTestClient.get().uri(builder -> builder.path("/{municipalityId}/endOfLeaseBatches/{batchId}").queryParam("status", "FAILED").build(MUNICIPALITY_ID, BATCH_ID))
			...
			.expectStatus().isOk()
			.expectBody()
			.jsonPath("$.total").isEqualTo(982)
			.jsonPath("$.computers.length()").isEqualTo(1)
			.jsonPath("$.computers[0].status").isEqualTo("FAILED");

		verify(endOfLeaseServiceMock).getBatchStatus(MUNICIPALITY_ID, BATCH_ID, List.of("FAILED"));
	}

	@Test
	void getEndOfLeaseBatchFilteredOnSeveralStates() {

		when(endOfLeaseServiceMock.getBatchStatus(MUNICIPALITY_ID, BATCH_ID, List.of("PENDING", "FAILED"))).thenReturn(EndOfLeaseBatchStatusResponse.create().withId(BATCH_ID));
		...
		verify(endOfLeaseServiceMock).getBatchStatus(MUNICIPALITY_ID, BATCH_ID, List.of("PENDING", "FAILED"));
	}
	...
	void getEndOfLeaseBatchAcceptsEveryStateThereIs(final EndOfLeaseStatus status) {
		...
		verify(endOfLeaseServiceMock).getBatchStatus(MUNICIPALITY_ID, BATCH_ID, List.of(status.name()));
	}
